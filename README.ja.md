...
TIFFのスケール：標準タグ（X/YResolution・ResolutionUnit）とOME-TIFFのPhysicalSizeX/Yに対応します。OME情報を優先し、詳細欄にµm/pixelと情報源を表示します。「設定・管理」でホバーのスケールバーを切り替えられます。既存TIFFは再生成して情報を取得してください。標準タグが印刷用DPIの場合は、その値に基づく長さとなり、標本の校正値とは限りません。メーカー独自タグには対応していません。
